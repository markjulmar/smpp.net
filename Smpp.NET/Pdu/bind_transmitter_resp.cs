...
namespace JulMar.Smpp.Pdu {
	/// <summary>
	/// The bind_transmitter_resp is sent in response to the bind_transmitter PDU.
	/// </summary>
	public class bind_transmitter_resp : SmppResponse {
		// Class data
		private system_id sid_ = new system_id();
		private sc_interface_version ver_ = new sc_interface_version(SmppVersion.SMPP_V50);

		/// <summary>
		/// Constructor for the bind transmitter response
		/// </summary>
		public bind_transmitter_resp()
			: base(Commands.BIND_TRANSMITTER_RESP) {
		...
		/// Constructor for the bind transmitter response
		/// </summary>
		public bind_transmitter_resp(int seqNum, int status)
			: base(Commands.BIND_TRANSMITTER_RESP, status, seqNum) {
			AddOptionalElements(ver_);
		}
		...
		/// Constructor for the bind transmitter response
		/// </summary>
		public bind_transmitter_resp(int seqNum, string sid)
			: base(Commands.BIND_TRANSMITTER_RESP, seqNum) {
			AddOptionalElements(ver_);
			SystemID = sid;
		}
		...
		public override string ToString() {
			return string.Format("bind_transmitter_resp: {0},sid={1},ver={2:X}{3}",
