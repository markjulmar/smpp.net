...
namespace JulMar.Smpp.Pdu {
	/// <summary>
	/// The submit_multi operation is used to submit an SMPP message for delivery to multiple
	/// receipients or to one or more distribution lists (DLs).  The submit_multi PDU does not
	/// support the transaction messaging mode.
	/// </summary>
	public class submit_multi : SmppSubmitRequest {
		// Class data - required parameters
		private service_type stype_ = new service_type();
		private address saddr_ = new address();
		private address_collection daddr_ = new address_collection();
		...
		/// </summary>
		public address_collection DestinationAddressCollection {
			get { return daddr_; }
		}
		...
		public override string ToString() {
			return string.Format("submit_multi: {0},svc_type={1},src_{2},dst_{3}," +
